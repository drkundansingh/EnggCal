...
enghub/
  ...
  js/
    ...
    calculators/          Pure calculation engine, ZERO DOM code, ZERO UI code
      units.mjs            Pressure/temp/flow/length/mass/power conversions
      transmitter.mjs       4-20mA / 0-20mA / 1-5V / 0-10V scaling
      dpLevel.mjs            DP-flow, hydrostatic & tank level
      orifice.mjs             Orifice plate flow (simplified ISO 5167)
      controlValve.mjs         Cv/Kv sizing (liquid/gas/steam, ISA-75.01-style)
      ipConverter.mjs           I/P transducer scaling
      rtd.mjs                    Pt100/Pt1000 (IEC 60751), Ni100/Cu100
      thermocouple.mjs            Type J/K/T/E/N/R/S/B, linear approx + CJC
      pid.mjs                      PID output, ZN/Cohen-Coon/IMC tuning
      thermalPlant.mjs              MW↔fuel↔steam estimator, config-driven
  data/formulaLibrary.json    Formula Library content (searchable)
  tests/test.mjs                Automated engine tests — `node tests/test.mjs`
```
...
same `js/calculators/*.mjs` files directly (they're plain ES modules with no
framework dependency) and wrap them in a new UI layer.

Assumptions are always separated from equations (spec #20): e.g.
`thermalPlant.mjs` exports `defaultAssumptions(plantType)` as a starting point,
...
`tests/test.mjs` is a real (not smoke) test suite — it checks each formula
...
`tests/accuracy_2000.mjs` is a large-scale (2000-point Monte Carlo) numerical
...
`tests/deep_10000.mjs` is a broader 10,000-point-per-check fuzz/property
...
to break silently since they touch the DOM/browser APIs that `tests/test.mjs`
can't reach (it only covers the pure calculation engine).

**Before you launch commercially**, add:
- A CI job that runs `node tests/test.mjs` on every commit
...
another calculator page. Reuses `flowEngine.mjs` entirely (same
...
in `flowEngine.mjs`, a standard reference-table interpolation (0-300°C,
...
A new, fully additive module (`js/calculators/flowEngine.mjs` + a new "Mode
...
`thermalPlant.mjs`) was verified to produce byte-identical results before
...
A new, fully additive module (`js/calculators/tripProtection.mjs` + a new
...
`tripProtection.mjs` is a flat array; adding more parameters is additive and
