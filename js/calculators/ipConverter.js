...
export function currentToPressure(mA, mAMin = 4, mAMax = 20, psiMin = 3, psiMax = 15) {
  if (![mA, mAMin, mAMax, psiMin, psiMax].every(Number.isFinite)) throw new Error('All signal and range values must be numbers.');
...
export function pressureToCurrent(psi, mAMin = 4, mAMax = 20, psiMin = 3, psiMax = 15) {
  if (![psi, mAMin, mAMax, psiMin, psiMax].every(Number.isFinite)) throw new Error('All signal and range values must be numbers.');
