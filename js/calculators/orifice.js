...
export function boreForFlow(targetQM3s, pipeDM, dpPa, densityKgM3, Cd = 0.6) {
  ...
  if (!(pipeDM > 0)) throw new Error('Pipe diameter must be > 0');
  if (!(dpPa > 0)) throw new Error('Differential pressure must be > 0');
  if (!(densityKgM3 > 0)) throw new Error('Density must be > 0');
  if (!(Cd > 0)) throw new Error('Discharge coefficient must be > 0');
